...
WEBHOOK_BODY = ''  # 自定义通知请求体，支持 $title 和 $content
WEBHOOK_HEADERS = ''  # 自定义通知请求头，按行填写 Header: Value
WEBHOOK_METHOD = ''  # 自定义通知请求方法，例如 POST、GET
...
if "WEBHOOK_URL" in os.environ and os.environ["WEBHOOK_URL"]:
    WEBHOOK_URL = os.environ["WEBHOOK_URL"]
if "WEBHOOK_BODY" in os.environ and os.environ["WEBHOOK_BODY"]:
    WEBHOOK_BODY = os.environ["WEBHOOK_BODY"]
if "WEBHOOK_HEADERS" in os.environ and os.environ["WEBHOOK_HEADERS"]:
    WEBHOOK_HEADERS = os.environ["WEBHOOK_HEADERS"]
if "WEBHOOK_METHOD" in os.environ and os.environ["WEBHOOK_METHOD"]:
    WEBHOOK_METHOD = os.environ["WEBHOOK_METHOD"]
if "WEBHOOK_CONTENT_TYPE" in os.environ and os.environ["WEBHOOK_CONTENT_TYPE"]:
    WEBHOOK_CONTENT_TYPE = os.environ["WEBHOOK_CONTENT_TYPE"]
...
if WEBHOOK_URL and WEBHOOK_METHOD:
    notify_mode.append('custom_notify')
    # print("企业微信机器人 推送打开")
...
def replace_webhook_vars(value, title, content):
    if not value:
        return value
    return value.replace("$title", title).replace("$content", content)


def parse_webhook_headers(headers, title, content):
    parsed_headers = {}
    if not headers:
    ...
    for line in headers.splitlines():
        ...
        parsed_headers[key.strip()] = replace_webhook_vars(value.strip(), title, content)
    return parsed_headers


def parse_webhook_key_values(value):
    parsed = {}
...
def parse_webhook_body(body, content_type, title, content):
    if not body:
        return None
    body = replace_webhook_vars(body, title.replace("\n", "\\n"), content.replace("\n", "\\n"))
    content_type = (content_type or "").lower()
    if content_type == "application/json":
        try:
            return json.dumps(json.loads(body), ensure_ascii=False).encode("utf-8")
        except Exception:
            parsed = parse_webhook_key_values(body)
            return json.dumps(parsed or {"title": title, "content": content}, ensure_ascii=False).encode("utf-8")
    if content_type == "application/x-www-form-urlencoded":
        params = parse_webhook_key_values(body)
        return urllib.parse.urlencode(params).encode("utf-8") if params else body.encode("utf-8")
    return body.encode("utf-8")


def custom_notify(title, content):
    if not WEBHOOK_URL or not WEBHOOK_METHOD:
        print("自定义通知的 WEBHOOK_URL 或 WEBHOOK_METHOD 未设置!!\n取消推送")
        return
    print("自定义通知服务启动")
    try:
        url = replace_webhook_vars(
            WEBHOOK_URL,
            urllib.parse.quote_plus(title),
            urllib.parse.quote_plus(content),
        )
        headers = parse_webhook_headers(WEBHOOK_HEADERS, title, content)
        if WEBHOOK_CONTENT_TYPE:
            headers.setdefault("Content-Type", WEBHOOK_CONTENT_TYPE)
        body = parse_webhook_body(WEBHOOK_BODY, WEBHOOK_CONTENT_TYPE, title, content)
        response = requests.request(
            method=WEBHOOK_METHOD.upper(),
            url=url,
            headers=headers,
            data=body,
            timeout=15,
    ...
    for i in notify_mode:
        ...
        if i == 'sc_key':
            ...
            if QYWX_KEY:
                ...
                print('未启用企业微信应用消息推送')
            continue
        elif i == 'custom_notify':
            if WEBHOOK_URL and WEBHOOK_METHOD:
                custom_notify(title=title, content=content)
            else:
                print('未启用自定义通知')
            continue
        else:
